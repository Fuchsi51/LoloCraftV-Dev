...
public class GalaxypigWennEntitatStirbtProcedure {

	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				LolocraftvMod.LOGGER.warn("Failed to load dependency world for procedure GalaxypigWennEntitatStirbt!");
			return;
		}
		IWorld world = (IWorld) dependencies.get("world");
		if (world.getWorldInfo().getGameRulesInstance().getBoolean(LcdebugGameRule.gamerule) == true) {
			LolocraftvMod.LOGGER.debug("[LC-V Debug INFO] Mod dead: Galaxy Pig");
		}
	}
}
