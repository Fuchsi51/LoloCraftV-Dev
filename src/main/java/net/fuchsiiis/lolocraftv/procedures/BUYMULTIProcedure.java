...
public class BUYMULTIProcedure {

	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				LolocraftvMod.LOGGER.warn("Failed to load dependency world for procedure BUYMULTI!");
			return;
		}
		if (dependencies.get("entity") == null) {
			if (!dependencies.containsKey("entity"))
				LolocraftvMod.LOGGER.warn("Failed to load dependency entity for procedure BUYMULTI!");
			return;
		}
					...
					for (int _idx = 0; _idx < _iitemhandlerref.get().getSlots(); _idx++) {
						...
						if (Amount_Of_Item_Needed > 15) {
							if (ItemBought == false) {
								...
								if (entity instanceof PlayerEntity) {
									ItemStack _setstack = new ItemStack(GalaxyMultiToolItem.block);
